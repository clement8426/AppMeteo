    ...
    if (!searches[city]) {
      ...
      if (userIP) {
        ...
        if (!previousSearches.includes(city)) {
          ...
          // Enregistrer la recherche avec son timestamp
          searches[city] = { count: searches[city].count + 1, timestamp: timestamp };
          localStorage.setItem("searches", JSON.stringify(searches));
        }
      }
    }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchRecentSearches() {
        fetch('/visitors/recent-searches')
            ...
            .catch(error => {
                console.error('Erreur lors de la récupération des 10 dernières recherches : ', error);
            });
    }
      ...
      function updateRecentSearches(data) {
        ...
        const currentTime = new Date().getTime(); // Temps actuel en millisecondes
          ...
          data.forEach((search, index) => {
            console.log("Timestamp:", search.timestamp); // Ajouter ce console.log pour afficher la valeur de search.timestamp
            ...
            searchDetails.textContent = `Country: ${search.country}, City: ${search.city}, Temperature: ${search.temperature}°C`;
            ...
            if (search.timestamp) {
              ...
              // Si c'est la dernière recherche, ajoutez le console.log
              if (index === data.length - 1) {
                console.log(`Timestamp for last search (${search.country}, ${search.city}): ${search.timestamp}`);
                console.log("Current Time:", currentTime);
                console.log("temps écoulé:", timeDiff);
